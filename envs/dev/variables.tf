...
# Inputs -- envs/dev
# =============================================================================
# Defaults here encode the dev posture. Values with no default are
...
variable "environment" {
  ...
  validation {
    ...
    error_message = "This root module is envs/dev; environment must be \"dev\". Applying it with another value would produce resource names that collide with the other environment in the same account."
  }
}

variable "azs" {
  description = "Availability zones. Two is acceptable in dev; prod uses three."
  type        = list(string)
}
...
variable "domain_name" {
  description = "Public FQDN this environment serves, e.g. dev.echo-pong.example.com."
  type        = string
}
...
variable "manage_account_globals" {
  description = <<-EOT
    ...
    second apply fails on EntityAlreadyExists. It is false here.
...
variable "waf_rule_action_override" {
  description = "Per-managed-rule-group action, \"count\" or \"block\". dev keeps every managed group in count mode permanently -- dev exists to surface false positives before prod flips them to block."
  type        = map(string)
  default = {
